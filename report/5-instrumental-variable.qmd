...
3. __Z__ should not directly influence __W__ (_exclusion restriction_).

 [ch. 23] gives an overview over three common instrumental variable methods: _indirect least squares_, _two-stage least-squares_, and _weak IV_.

#### Wald estimator

In the case of a single binary instrument and a single treatment, we can use the Wald estimator [@dingFirstCourseCausal2023, ch. 21]. For the relaxed case -- where the instrument is not necessarily binary -- it is also known as indirect least squares [@dingFirstCourseCausal2023, ch. 23.6.2], or simply as instrumental variable method.

##### Deduction from structural equation model
...
$$
W = \beta + \beta_Z Z + \beta_U U
$$

Reduced form:

$$
Y = \gamma + \gamma_Z Z + \gamma_U U
$$

Structural equation:

$$
Y = \alpha + \alpha_W W + \alpha_U U
$$
...
$$
\begin{aligned}
W &= \alpha + Cov(W,Z) Z + \alpha_U U && \text{First stage} \\
Y &= \gamma + Cov(Y,Z) Z + \gamma_U U && \text{Reduced form} \\
Y &= \beta + \beta_W W + \beta_U U && \text{Structural equation} \\

Y &= \gamma + Cov(Z,\beta + \beta_W W + \beta_U U) Z + \gamma_U U && \text{Substitute structural equation into reduced form} \\
&= \gamma + \beta_W Cov(Z, W) Z + \gamma_U U \\

0 &= \beta_W Cov(Z,W) Z - Cov(Y, Z) Z && \text{Subtract from reduced form} \\
&= (\beta_W Cov(Z,W) - Cov(Y,Z)) Z \\
&= (\beta_W Cov(Z,W) - Cov(Y,Z)) && Assert Z \neq 0 \\

\beta_W &= \frac{Cov(Y,Z)}{Cov(Z,W)}
\end{aligned}
$$

In @eq-reg-beta we had a regression model of the same structure, and showed that the coefficient for the treatment is an estimate of the CATT. Hence $\tau_{g,d}=\frac{Cov(Y_d,Z)}{Cov(Z,W_g)}$. Estimators for the variance are given in @dingFirstCourseCausal2023 [ch. 21]. Impressively, the instrument allows us to control for confounders without needing to specify them.
...
##### Deduction from complier setting

[@dingFirstCourseCausal2023, ch. 21; @pischkeLabourEconomicsPhD2019, slides on the LATE theorem; facurealvesCausalInferenceBrave2022, ch. 9]
