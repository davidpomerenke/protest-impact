...
Y = \gamma + \gamma_Z Z + \gamma_U U && text{Reduced form} \\
\end{aligned}
$$
...
If randomization holds only conditionally, then we need to control for all covariates conditional to which the instrument is random. Adding controls is also useful to increase statistical power. @torgovitskyWhenTSLSActually point out widespread problems with this approach.
