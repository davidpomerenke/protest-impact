...
| Model          | Hyp. opt. obj. | RMSE          | AME          |
| -------------- | -------------- | ------------- | ------------ |
| OLS            | min RMSE       | 110.6 ± 24.2  | -18.8 ± 10.4 |
| OLS            | min AME        | 130.9 ± 21.5  | -14.3 ± 10.1 |
| BayesianRidge  | min AME        | 119.7 ± 23.4  | -3.85 ± 15.2 |
| LassoLarsIC    | min AME        | 118.9 ± 24.5  | -2.89 ± 15.7 |

: Average mean error (AME) as a measure for bias, and root mean squared error (RMSE) as a measure for variance for the best models that result from hyperparameter optimization. {#tbl-reg-hypopt}
...
```{python}
# | label: tbl-iv-wald
# | tbl-cap: Correlations between the instruments and the treatment (corr_w) and between the instruments and the outcome (corr_y), and Wald estimator results.
# | echo: false
...
Correlations and Wald estimator results of the original variable are shown in @tbl-iv-wald. Note that the Wald estimator is not valid for weak instruments.
...
```{python}
# | label: tbl-iv-basic
# | tbl-cap: First-stage impact of the original instruments. Regression coefficients and p-values for the impact of the standardized instruments on the treatment, controlling for known confounders. Results are sorted by original p-values and adjusted p-values (p_bh = Benjamini-Hochberg, p_by=Benjamini-Yekutieli) and first-stage f-statistics for using each variable as an instrument on its own are displayed.
# | echo: False
...
# | tbl-cap: First-stage impact of principal components calculated separately for the seasonal and residual parts of the instruments. Regression coefficients and p-values for the impact of the standardized instruments on the treatment, controlling for known confounders. Results are sorted by original p-values and adjusted p-values (p_bh = Benjamini-Hochberg, p_by=Benjamini-Yekutieli) and first-stage f-statistics for using each variable as an instrument on its own are displayed. Only the 10 most significant variables are displayed.
# | echo: False
...
In the _ACLED_ dataset there is a protest in every single region on March 19, 2021, so no control region can be constructed. For the _German Protest Reports_ dataset this also occurs on September 9, 2021 and on March 25, 2022.
...
__Propensity scores from text.__ Model finetuning takes roughly 1.5 hours on 8 RTX 4090 GPUs, at a rental cost of roughly 5 euros. Loss curves are shown in @fig-loss-curves and suggest that the model would not profit from further training epochs. The F1-score plateaus around 0.23, with the best checkpoint at F1=0.238. (No cross-validation has been performed, so no uncertainty can be stated.) Since this is not better than the simple model above, I use propensity scores from the simple model for the computation of the causal estimates.
...
fig, axes = plot_groups(kind="methods", step=6)
axes[0].set_ylim(-60, 180)
axes[1].set_ylim(-30, 90)
axes[2].set_ylim(-4, 12)
axes[3].set_ylim(-4, 12)
axes[4].set_ylim(-1000, 3000)
...
fig, axes = plot_groups(kind="groups", step=6)
axes[0].set_ylim(-15, 25)
fig.set_size_inches(12,3)
plt.show()
```

@fig-impact-dataset and @fig-impact-groups-sources show how the impact estimates differ by dataset, for using the synthetic control method. Detailed results for the _German Protest Registrations_ and _German Protest Reports_ datasets for all causal methods are in the appendix in @fig-impact-ts-gpreg and @fig-impact-ts-gprep, respectively.
...
# | fig-cap: 7-day cumulative causal impact estimates by dataset, as estimated by the synthetic control method. Error bars are 95% confidence intervals. Pay attention to differing y-axis scaling.
# | echo: false

from src.visualization.impacts import plot_groups
import matplotlib.pyplot as plt

fig, axes = plot_groups(kind="sources", step=6)
