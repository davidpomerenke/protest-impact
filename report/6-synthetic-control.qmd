...
E\left(\sum_{s \in R_0} \gamma_s X_s\right) = X_r && \text{Interpolation} \label{eq-interpol}
\end{align}
...
&= \beta + \beta_X E\left(\sum_{s \in R_0} \gamma_s  X_s\right) + \beta_U U + \sum_{i=0}^{|X_r|} \sum_{j=0}^{|U|} \beta_{UX,ij} \left[E\left(\sum_{s \in R_0} \gamma_s  X_s\right) U^T\right]_{ij} && \text{by assumption \eqref{eq-convex}} \\
&= \beta + \beta_X X_r + \beta_U U + \sum_{i=0}^{|X_r|} \sum_{j=0}^{|U|} \beta_{UX,ij} (X_r U^T)_{ij} && \text{by assumption \eqref{eq-interpol}} \\
&= Y_r(0)
\end{align}
:::
