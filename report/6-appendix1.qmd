...
Detailed regression results are in @tbl-reg-detailed and @tbl-reg-details-occ.

```{python}
# | echo: False
# | label: tbl-reg-detailed
# | tbl-cap: Regression tables.
# | tbl-subcap:
...
tables_ = best_regression().summary().tables
results_as_html = tables_[1].as_html()
df = pd.read_html(results_as_html, header=0, index_col=0)[0]
df_ = df.sort_values("P>|z|").head(20).sort_index()
display(df_)
# df_.to_csv(tables / "regtable.csv", index=False)
...
@fig-loss-curves shows the loss curves for finetuning the transformer text classification model.

The following is an example text that has been used for the finetuning process. This text has the label `no`. The format is conceived so that it could also be used with _causal_ language models^[This is confusing terminology: A causal language model is not related to causal inference, but merely predicts a word given the previous words, and this is called "causal" in natural language processing.], but here I train a _classification_ language model, because it is more straightforward to convert the classification logits to probabilities. (For a classification model there are only two possible classes, while a causal model can answer with any word from its vocabulary, but only two of them correspond to the class labels.)
...
ax1.plot(
    training_steps,
    pd.Series(training_loss_values).rolling(10).mean(),
    label="training loss",
)
...
#### Impact estimates for the other datasets {#sec-app1-datasets}
...
fig.suptitle(
    "German Protest Registrations dataset: Time series of the causal impact by method and coverage dimension"
)
...
fig.suptitle(
    "German Protest Reports dataset: Time series of the causal impact by method and coverage dimension"
)
axes[0].set_ylim(-30, 30)
axes[1].set_ylim(-30, 30)
axes[2].set_ylim(-5, 5)
