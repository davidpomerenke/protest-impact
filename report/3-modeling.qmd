...
We can group the variables from the dataset described in @sec-data into 4 causal categories, and add another category for variables that we do not know but that might be relevant. For each day and each region, we have data in the following categories:

- Treatment __W__:
  - For each protest group, a dummy variable whether it organizes at least one protest event in the given region (_binary treatment_)
  - Alternatively, for each protest group the number of protesters in the given region (_continuous treatment_)
- Outcome __Y__:
  - Number of newspaper articles published, across several dimensions (see @sec-data-discourse)
- Instruments __Z__:
  - Weather: min/max/avg temperature, air pressure, precipitation, snowfall, wind speed, and peak gust; with the longterm climate components removed, respectively (see @sec-instrumental for explanation and discussion)
...
- Known confounders __X__:
  ...
  - Climate: climate components of the weather variables (see above)
...
- __Historic variables:__ Variables that are only available for past dates but not for the date of the prediction. This is especially true for the target variable; since its future value should be predicted, it should not be present in the training data.
...
We can now use these notions to define the machine learning tasks for the different methods in terms of time series prediction.

#### Regression

Predict __Y__ using:

- __W__ as future variable [^notation]
- __X__ as future and static variables
- __Y__ as historic variable

[^notation]: The past values of the treatment __W__ are then technically not a part of the treatment, but rather a part of the covariates __X__ (see @fig-causal-graph-time-series). This is analogously true for the past values of __Y__ and __Z__, both for regression as well as for the other methods below.
...
The causal effect can be estimated __directly__ (only in the case of a single instrumental variable and linear homogenous effects) ...

...

#### Two-stage procedure

... or it can be estimated a more flexible two-stage procedure, such as two-stage least squares or causal random forests. -->

Here we typically use a two-stage procedure, for which different machine learning implementations exist.

(First stage.) Predict __W__ using:

- __Z__ as future variable
- __X__ as future and static variables
- __Y, W__ as historic variables

Use the estimates $\hat W$ for the second stage.

(Second stage.) Predict __Y__ using:

- $\hat W$ as static variable^[$\hat W$ in combination with the past values of __W__ form a future variable then.]
- __X__ as future and static variables
- __Y, W__ as historic variables

Interpret the resulting coefficient for $\hat W$ causally.
...
- __X, Y, W__ from the prediction region and the control regions as static and historic values, for weighting purposes
...
| ------ | ---------- | ------ |
| __Y__ | $X_{f,s}, Y_h, W_f$ | Regression |
| __Y__ | $X_{f,s}, Y_h, W_h$ | Double ML |
| __Y__ | $X_{f,s}, Y_h, W_h, \hat W_s$ | IV stage 2 |
| __Y__ | $X_{f,s}, Y_h, W_h, Y_{control f}$ |  SC counterfactual |
| __W__ | $X_{f,s}, Y_h, W_h$ | Propensity score, Double ML |
| __W__ | $X_{f,s}, Y_h, W_h, Z_f$ | IV stage 1 |

<!-- | __Y__ | $X_{f,s}, Y_h, W_h$ | use estimates | Counterfactual for synthetic control (version 2) | -->
